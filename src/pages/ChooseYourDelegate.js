...
const processENSDelegateContractResults = (results, delegateData) =>
  results?.map((result) => {
    const data = delegateData.find(
      (data) => data.addr.id === result.addr.toLowerCase()
    )
    return {
...
const useGetDelegates = (isConnected) => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const delegateNamehashes = delegateData.resolvers
        .map((result) => namehash(result.domain.name))
      ...
      setDelegates(
        processENSDelegateContractResults(results, delegateData?.resolvers)
      )
    }
    try {
      if (isConnected) {
